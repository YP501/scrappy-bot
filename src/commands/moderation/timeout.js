...
const info = new SlashCommandBuilder()
    ...
    .addStringOption(option => option
        .setName('time')
        .setDescription('10s = 10 seconds, 10m = 10 minutes, 10h = 10 hours, 10d = 10 days')
        .setRequired(true))
    .addStringOption(option => option
        .setName('reason')
...
const execute = async (inter) => {
    ...
    try {
        ...
        await inter.editReply('Cancelled command');
        await inter.followUp({ content: "An error occured while timing out the user, they might already be muted or I don't have the permission to mute them", ephemeral: true });
        return;
    };
